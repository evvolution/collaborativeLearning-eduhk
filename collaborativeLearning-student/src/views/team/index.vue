  ...
  <div class="res-view commonView">
    <div class="res-list common-scroll">
      <t-list-item 
        ...
        @click="selectCase(index, item.id, item.closeByAdmin == 1 ? false : true)"
      >{{item.name}}
      </t-list-item>
    </div>
    <div class="res-panel" v-if="caseValue">
      <t-space style="margin-bottom: 12px;" v-if="currentAllowUpload">
        <t-upload
        ...
        />
        <t-button theme="primary" :disabled="teamworkList.length == 0" @click="handle_add">SUBMIT</t-button>
      </t-space>
...
import { addHomework, getCaseList, getHomeworkList } from '@/api/modules/case'
...
const currentAllowUpload = ref()
const selectCase = (index: any, value: any, state: any) => {
  currentIndex.value = index
  caseValue.value = value
  currentAllowUpload.value = state
  getList(1, 10)
}
