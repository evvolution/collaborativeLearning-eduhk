<template>
  ...
  >
    ...
    <template #action="{ row }">
      <t-switch v-model="row.opened" :customValue="[1, 0]" :label="['Pubilc','Private']" @change="changeRes(row)"></t-switch>
    </template>
  </t-table>
...
import { ref } from 'vue'
import { getGroupResByCaseId, updateResources } from '@/api/modules/resource'
...
const columns = ref([
  ...
  { colKey: 'creator', title: 'Original Creator', width: 150 },
  { colKey: 'content', title: 'Content' },
  { colKey: 'attachment', title: 'Attachment', width: 120 },
