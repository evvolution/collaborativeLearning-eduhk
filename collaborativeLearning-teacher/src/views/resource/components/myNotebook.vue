...
import { ref } from 'vue'
import { addResources, deleteResources, getGroupResByCaseId } from '@/api/modules/resource'
...
// add
const addToFactors = (item: any) => {
  let preJson = JSON.parse(item.content)
  preJson.creator = item.teacherName
  addResources({
    caseId: props.caseId,
    type: 2,
    opened: 1,
  ...
  .then((res: any)=> {
    if (res.code == 200) {
      return NotifyPlugin.success({
        ...
        content: "You can view in 'Teaching Factors' tab"
      })
    } else {
